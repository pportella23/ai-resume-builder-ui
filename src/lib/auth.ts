...
const USE_MOCK = process.env.NEXT_PUBLIC_USE_MOCK === 'true' || !API_BASE_URL || (API_BASE_URL && API_BASE_URL.trim() === '')

export const authOptions = {
  secret: process.env.NEXTAUTH_SECRET || process.env.AUTH_SECRET || 'mock-secret-for-development',
  providers: [
    GoogleProvider({
    ...
    CredentialsProvider({
      ...
      async authorize(credentials) {
        ...
        const emailMatch = credentials.email.toLowerCase().trim() === DEMO_USER.email.toLowerCase().trim()
